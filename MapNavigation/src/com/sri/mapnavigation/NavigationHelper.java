...
public class NavigationHelper {
	...
	private NavigationListener pNavigationListener;
	...
	public void setPNavigationListener(NavigationListener listener) {
	...
	private void startService() {
		if (!isNavServiceRunning(activity)) {
			...
			activity.startService(new Intent(activity, NavigationService.class));
		}
	}
	...
	private void sendInitialAction() {
		Intent i = new Intent();
		i.putExtra(NavigationHelper.NAV_ACTION, NavigationHelper.ACTIVITY_RUNNING);
		sendBroadCast(i);
	}
	...
	private void registerReceiver() {
		...
		filter.addAction(NavigationHelper.ACTION_HELPER_SERVICE);
		activity.registerReceiver(helperReceiver, filter);
	}

	public void unregisterReceiver() {
		Intent i = new Intent();
		i.putExtra(NavigationHelper.NAV_ACTION, NavigationHelper.ACTIVITY_STOPPED);
		sendBroadCast(i);
	...
	private void sendBroadCast(Intent i) {
		...
		i.setAction(NavigationHelper.ACTION_NAV_SERVICE);
	...
	private BroadcastReceiver helperReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context paramContext, Intent paramIntent) {
			...
			if (action.equals(NavigationHelper.ACTION_HELPER_SERVICE)) {
	...
	private void doAction(Intent data) {
		...
		switch (action) {
		case NavigationHelper.SERVICE_RUNNING:
			L.i(TAG, "service running");
			sendInitialAction();
			checkGPS();
			callActivityResumed();
			break;
		case NavigationHelper.GPS_OFF:
			L.i(TAG, "GPS Off");
			showGPSDialog();
			break;
		case NavigationHelper.GPS_ON:
			L.i(TAG, "GPS On");
			dismissGPSDialog();
			startGPS();
			break;
		case NavigationHelper.ACTION_LOCATION_CHANGED:
//			L.i(TAG, "Location changed");
			actionLocationChanged((Location) data.getExtras().getParcelable(NavigationHelper.CHANGED_LOCATION));
			break;
		case NavigationHelper.ACTION_GPS_PROVIDER_STATUS_CHANGED:
//			L.i(TAG, "GPS status changed");
			actionGPSProviderStatusChanged(data.getExtras().getInt(NavigationHelper.CHANGED_GPS_PROVIDER_STATUS));
			break;
		case NavigationHelper.ACTION_GPS_STATUS_CHANGED:
//			L.i(TAG, "GPS status changed");
			actionGPSStatusChanged(data.getExtras().getInt(NavigationHelper.CHANGED_GPS_STATUS));
			break;
		case NavigationHelper.ACTION_SENSOR_CHANGED:
			// L.i(TAG, "Sensor changed");
			actionSensorChanged(data.getExtras().getFloat(NavigationHelper.CHANGED_SENSOR));
			break;
		}
	}
	...
	public void checkGPS() {
		Intent i = new Intent();
		i.putExtra(NavigationHelper.NAV_ACTION, NavigationHelper.ACTION_CHECKGPS);
		sendBroadCast(i);
	}

	private void startGPS() {
		Intent i = new Intent();
		i.putExtra(NavigationHelper.NAV_ACTION, NavigationHelper.ACTION_STARTGPS);
		sendBroadCast(i);
	}
	...
	public static boolean isNavServiceRunning(Context context) {
		...
		for (RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
			if (NavigationService.class.getName().equals(service.service.getClassName())) {
				return true;
			}
		}
		return false;
...
//	}
	...
	public void callActivityResumed() {
		Intent i = new Intent();
		i.putExtra(NavigationHelper.NAV_ACTION, NavigationHelper.ACTIVITY_RESUMED);
		sendBroadCast(i);
	}

	public void callActivityPaused() {
		Intent i = new Intent();
		i.putExtra(NavigationHelper.NAV_ACTION, NavigationHelper.ACTIVITY_PAUSED);
		sendBroadCast(i);
	}
